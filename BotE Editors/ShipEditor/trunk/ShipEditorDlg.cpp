...
void CShipEditorDlg::DataToDialog()
{
	if (m_ShipInfo.GetSize() > 0)
	{
		...
		{
			CString s;
			s.Format("%d x %s Type %d [%d-%d]",m_ShipInfo.GetAt(m_ListBox.GetCurSel())->GetBeamWeapons()->GetAt(i).GetBeamNumber(),
									...
									m_ShipInfo.GetAt(m_ListBox.GetCurSel())->GetBeamWeapons()->GetAt(i).GetFirearc()->GetPosition(),
									m_ShipInfo.GetAt(m_ListBox.GetCurSel())->GetBeamWeapons()->GetAt(i).GetFirearc()->GetAngle());
		...
		for (int i = 0; i < m_ShipInfo.GetAt(m_ListBox.GetCurSel())->GetTorpedoWeapons()->GetSize(); i++)
			{
				CString s;
				s.Format("%d x %s [%d-%d]",m_ShipInfo.GetAt(m_ListBox.GetCurSel())->GetTorpedoWeapons()->GetAt(i).GetNumberOfTupes(),
									m_ShipInfo.GetAt(m_ListBox.GetCurSel())->GetTorpedoWeapons()->GetAt(i).GetTupeName(),
									m_ShipInfo.GetAt(m_ListBox.GetCurSel())->GetTorpedoWeapons()->GetAt(i).GetFirearc()->GetPosition(),
									m_ShipInfo.GetAt(m_ListBox.GetCurSel())->GetTorpedoWeapons()->GetAt(i).GetFirearc()->GetAngle());
...
void CShipEditorDlg::OnModifybeam() 
{
	...
	if (pDlg->DoModal() == IDOK)
	{
		...
		weaponMod->GetFirearc()->SetValues(pDlg->m_nMountPos, pDlg->m_nAngle);
		...
		for (int i = 0; i < m_ShipInfo.GetSize(); i++)
		{
			for (int j = 0; j < m_ShipInfo.GetAt(i)->GetBeamWeapons()->GetSize(); j++)
			{
				...
				{	
					...
					weaponNew->GetFirearc()->SetValues(
						weaponNew->GetFirearc()->GetPosition(),	// bleibt gleich
						weaponMod->GetFirearc()->GetAngle());	// anpassen
				}
			}
		}
		DataToDialog();
...
void CShipEditorDlg::OnAddbeam() 
{
	...
	if (m_ListBox.GetCurSel() != CB_ERR)
	{
		...
		if (pDlg->DoModal() == IDOK)
		{
			...
			new_beam.GetFirearc()->SetValues(pDlg->m_nMountPos, pDlg->m_nAngle);
...
void CShipEditorDlg::OnAddtube() 
{
	...
	if (m_ListBox.GetCurSel() != CB_ERR)
	{
		...
		if (pDlg->DoModal() == IDOK)
		{
			...
			new_torpedo.GetFirearc()->SetValues(pDlg->m_nMountPos, pDlg->m_nAngle);
