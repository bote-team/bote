...
BOOL CModifyTubeDlg::OnInitDialog() 
{
	...
	if (m_nClickedTube != CB_ERR)
	{
		...
		m_nMountPos			= weapon->GetFirearc()->GetPosition();
		m_nAngle			= weapon->GetFirearc()->GetAngle();
	}
	else
	{
		...
		m_byAccuracy		= 50;
		CFireArc fireArc;
		m_nMountPos			= fireArc.GetPosition();
		m_nAngle			= fireArc.GetAngle();
	}

	for (int i = 0; i < DIFFERENT_TORPEDOS; i++)
	{
		CString sTorp;
		sTorp.Format("%s - (Dmg.: %d)", CTorpedoInfo::GetName(i), CTorpedoInfo::GetPower(i));
		m_TorpedoTypeList.AddString(sTorp);
	}
