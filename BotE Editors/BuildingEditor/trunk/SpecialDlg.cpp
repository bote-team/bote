	...
	: CDialog(CSpecialDlg::IDD, pParent)
{
	...
	m_iShipbuildSpeed = 0;
	m_bTitanDistributor = FALSE;
	m_bDeuteriumDistributor = FALSE;
	m_bDuraniumDistributor = FALSE;
	m_bCrystalDistributor = FALSE;
	m_bIridiumDistributor = FALSE;
	m_bDeritiumDistributor = FALSE;
}
...
void CSpecialDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_TITANDIST, m_bTitanDistributor);
	DDX_Check(pDX, IDC_DEUTERIUMDIST, m_bDeuteriumDistributor);
	DDX_Check(pDX, IDC_DURANIUMDIST, m_bDuraniumDistributor);
	DDX_Check(pDX, IDC_CRYSTALDIST, m_bCrystalDistributor);
	DDX_Check(pDX, IDC_IRIDIUMDIST, m_bIridiumDistributor);
	DDX_Check(pDX, IDC_DERITIUMDIST, m_bDeritiumDistributor);
}
...
void CSpecialDlg::DataToDialog(CBuildingInfo* building, int language)
{
	...
	m_bTitanDistributor = building->GetResourceDistributor(TITAN);
	m_bDeuteriumDistributor = building->GetResourceDistributor(DEUTERIUM);
	m_bDuraniumDistributor = building->GetResourceDistributor(DURANIUM);
	m_bCrystalDistributor = building->GetResourceDistributor(CRYSTAL);
	m_bIridiumDistributor = building->GetResourceDistributor(IRIDIUM);
	m_bDeritiumDistributor = building->GetResourceDistributor(DILITHIUM);
...
void CSpecialDlg::DialogToData(CBuildingInfo* building, int language)
{
	...
	building->SetResourceDistributor(TITAN, m_bTitanDistributor == TRUE);
	building->SetResourceDistributor(DEUTERIUM, m_bDeuteriumDistributor == TRUE);
	building->SetResourceDistributor(DURANIUM, m_bDuraniumDistributor == TRUE);
	building->SetResourceDistributor(CRYSTAL, m_bCrystalDistributor == TRUE);
	building->SetResourceDistributor(IRIDIUM, m_bIridiumDistributor == TRUE);
	building->SetResourceDistributor(DILITHIUM, m_bDeritiumDistributor == TRUE);
}