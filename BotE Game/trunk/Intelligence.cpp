...
void CIntelligence::Serialize(CArchive &ar)
{
	...
	{
		if (m_Reports.GetReport(i)->GetOwner() == m_byRace)
...
void CIntelligence::AddSPStoragePoints(BOOLEAN type, BYTE race, int add)
{
	ASSERT(race);

	if (((long)m_iSPStorage[race-1][type] + add) < NULL)
		m_iSPStorage[race-1][type] = NULL;
	else
		m_iSPStorage[race-1][type] += add;
}
...
void CIntelligence::Reset()
{
	for (int i = 0; i < DOMINION; i++)
	{
		memset(m_iSPStorage[i], 0, sizeof(*m_iSPStorage[i])*2);
		memset(m_byAggressiveness[i], 1, sizeof(*m_byAggressiveness[i])*2);
	}
	m_byRace = NOBODY;
	m_byResponsibleRace = NOBODY;
	m_iSecurityPoints = 0;
	m_iInnerStorage = 0;
	m_Assignment.Reset();
