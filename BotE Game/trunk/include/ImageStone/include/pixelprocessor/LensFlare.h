...
@endverbatim
 */
class FCPixelLensFlare : public FCSinglePixelProcessBase
{
public:
    /**
     *  Constructor.
     *  @param pt : coordinate on image
     */
    FCPixelLensFlare (POINT pt)
    {
        m_pt = pt ;
    }
private:
    ...
    static void fixpix (BYTE* p, const double& procent, const FLOATRGB& colpro)
    {
    ...
    {
        double   procent = h - m_shalo ;
        procent /= (m_shalo * 0.07) ;
        procent = fabs(procent) ;
        if (procent < 1)
            fixpix (s, 1 - procent, m_halo) ;
    }
    void mrt1 (BYTE* s, Reflect* ref, int x, int y)
    {
        double   procent = ref->size - FHypot (ref->xp - x, ref->yp - y) ;
        procent /= ref->size ;
        if (procent > 0)
        {
            procent *= procent ;
    ...
    void mrt2 (BYTE* s, Reflect* ref, int x, int y)
    {
        double   procent = ref->size - FHypot (ref->xp - x, ref->yp - y) ;
    ...
    void mrt3 (BYTE* s, Reflect* ref, int x, int y)
    {
        double   procent = ref->size - FHypot (ref->xp - x, ref->yp - y) ;
    ...
    void mrt4 (BYTE* s, Reflect* ref, int x, int y)
    {
        double   procent  = FHypot (ref->xp - x, ref->yp - y) - ref->size ;
    ...
    virtual void OnEnterProcess (FCObjImage* pImg)
    {
        ...
        initref (m_pt.x, m_pt.y, pImg->Width(), pImg->Height(), matt) ;
    }
