				...
				if (atoi(s) == m_byType)
				{
					...
					if (atoi(s) == n)
					{
						...
						if (n != 3)
						{
							...
							{
								s = pDoc->GetRaceCtrl()->GetMinorRace(pDoc->GetSystem(m_MinorRaceKO.x, m_MinorRaceKO.y).GetName())->GetName();
						...
						else
						{
							...
							if (m_sMajor != "")
							{
								...
								s = CLoc::GetString("FEMALE_ARTICLE")+" " + pDoc->GetRaceCtrl()->GetMinorRace(pDoc->GetSystem(m_MinorRaceKO.x, m_MinorRaceKO.y).GetName())->GetName();
							csInput.Replace("$major$", s);
