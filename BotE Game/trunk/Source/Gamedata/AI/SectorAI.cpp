...
void CSectorAI::CalcualteSectorPriorities()
{
	...
	for (int x = 0; x < STARMAP_SECTORS_HCOUNT; x++)
		for (int y = 0; y < STARMAP_SECTORS_VCOUNT; y++)
		{
			...
			if (m_pDoc->GetSystem(x, y).GetSunSystem())
			{
	...
	for (CRaceController::const_iterator it = race_ctrl.begin(); it != race_ctrl.end(); ++it)
		if (it->second->IsMajor())
		{
			...
			if(MT::CMyTrace::IsLoggingEnabledFor("ai"))
			{
				...
				{
					MYTRACE_CHECKED("ai")(MT::LEVEL_DEBUG, "\n---------- sectors to terraform or colonize -------------\n");
...
void CSectorAI::AddDanger(const boost::shared_ptr<const CShips>& ship)
{
	...
	m_iDangers[race][pair<int, int>(ship->GetCo().x, ship->GetCo().y)] += (offensive + defensive);

	if (ship->GetShipType() > SHIP_TYPE::COLONYSHIP && ship->GetShipType() < SHIP_TYPE::OUTPOST)
	{
		m_iCompleteDanger[race] += (offensive + defensive);
		m_iCombatShipDangers[race][pair<int, int>(ship->GetCo().x, ship->GetCo().y)] += (offensive + defensive);
	}
		...
		for (map<CString, CMajor*>::const_iterator it = pmMajors->begin(); it != pmMajors->end(); ++it)
			if (it->second->GetStarmap()->GetRange(CPoint(x,y)) != 3)
				if (it->second->IsRaceContacted(pOwner->GetRaceID()) == false)
					if (pOwner->GetCo() != CPoint(-1,-1))
						m_vMinorraceSectors[it->first].push_back(pOwner->GetCo());
}
