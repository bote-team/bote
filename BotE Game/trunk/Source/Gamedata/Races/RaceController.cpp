		...
		for (map<CString, CRace*>::const_iterator jt = m_mRaces.begin(); jt != m_mRaces.end(); jt++)
			...
			if (it->first != jt->first && jt->second->GetType() == MAJOR)
			{
				...
				if (it->second->IsRaceProperty(FINANCIAL))
				{
					...
					if (jt->second->IsRaceProperty(FINANCIAL))
						nBoni += 25;
					if (jt->second->IsRaceProperty(WARLIKE))
						nBoni += 5;
					if (jt->second->IsRaceProperty(AGRARIAN))
						nBoni += 10;
					if (jt->second->IsRaceProperty(INDUSTRIAL))
						nBoni += 15;
					if (jt->second->IsRaceProperty(SECRET))
						nBoni -= 15;
					if (jt->second->IsRaceProperty(SCIENTIFIC))
						nBoni += 5;
					if (jt->second->IsRaceProperty(PRODUCER))
						nBoni += 15;
					if (jt->second->IsRaceProperty(PACIFIST))
						nBoni -= 25;
					if (jt->second->IsRaceProperty(SNEAKY))
						nBoni -= 20;
					if (jt->second->IsRaceProperty(SOLOING))
						nBoni -= 25;
					if (jt->second->IsRaceProperty(HOSTILE))
						nBoni -= 40;
				}
