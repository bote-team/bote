...
void CVictoryObserver::Init(void)
{
	...
	MYTRACE_DOMAIN("general")(MT::LEVEL_INFO, "Victory_Conditions - Elimination: %s", m_bConditionStatus[VICTORYTYPE_ELIMINATION] ? "true" : "false");
	pIni->ReadValue("Victory_Conditions", "Diplomacy", m_bConditionStatus[VICTORYTYPE_DIPLOMACY]);
	MYTRACE_DOMAIN("general")(MT::LEVEL_INFO, "Victory_Conditions - Diplomacy: %s", m_bConditionStatus[VICTORYTYPE_DIPLOMACY] ? "true" : "false");
	pIni->ReadValue("Victory_Conditions", "Conquest", m_bConditionStatus[VICTORYTYPE_CONQUEST]);
	MYTRACE_DOMAIN("general")(MT::LEVEL_INFO, "Victory_Conditions - Conquest: %s", m_bConditionStatus[VICTORYTYPE_CONQUEST] ? "true" : "false");
	pIni->ReadValue("Victory_Conditions", "Research", m_bConditionStatus[VICTORYTYPE_RESEARCH]);
	MYTRACE_DOMAIN("general")(MT::LEVEL_INFO, "Victory_Conditions - Research: %s", m_bConditionStatus[VICTORYTYPE_RESEARCH] ? "true" : "false");
	pIni->ReadValue("Victory_Conditions", "Combat", m_bConditionStatus[VICTORYTYPE_COMBATWINS]);
	MYTRACE_DOMAIN("general")(MT::LEVEL_INFO, "Victory_Conditions - Combat: %s", m_bConditionStatus[VICTORYTYPE_COMBATWINS] ? "true" : "false");
	pIni->ReadValue("Victory_Conditions", "Sabotage", m_bConditionStatus[VICTORYTYPE_SABOTAGE]);
	MYTRACE_DOMAIN("general")(MT::LEVEL_INFO, "Victory_Conditions - Sabotage: %s", m_bConditionStatus[VICTORYTYPE_SABOTAGE] ? "true" : "false");
}
...
int CVictoryObserver::GetNeededVictoryValue(VICTORYTYPE nType) const
{
	...
	case VICTORYTYPE_DIPLOMACY:
		...
		nValue--;
		MYTRACE_DOMAIN("general")(MT::LEVEL_INFO, "VICTORYTYPE_DIPLOMACY - needed value: %i", nValue/2);
		...
		for (int y = 0 ; y < STARMAP_SECTORS_VCOUNT; y++)
			for (int x = 0; x < STARMAP_SECTORS_HCOUNT; x++)
				if (pDoc->GetSector(x,y).GetCurrentHabitants() > 0.0 && pDoc->GetSector(x,y).GetOwnerOfSector() != "")
					nValue++;
		MYTRACE_DOMAIN("general")(MT::LEVEL_INFO, "VICTORYTYPE_CONQUEST - needed value: %i", nValue/2);
		return max(nValue >> 1, 10);
	...
	case VICTORYTYPE_COMBATWINS:
		...
		//MYTRACE_DOMAIN("general")(MT::LEVEL_INFO, "VICTORYTYPE_COMBATWINS - needed value: %i", nValue);
	...
	if (m_bConditionStatus[VICTORYTYPE_ELIMINATION])
	{
		...
		for (map<CString, CMajor*>::const_iterator it = pmMajors->begin(); it != pmMajors->end(); ++it)
			if (it->second->GetEmpire()->GetNumberOfSystems())
				nMajorsAlive++;
		MYTRACE_DOMAIN("general")(MT::LEVEL_INFO, "nMajorsAlive: %i", nMajorsAlive);
