...
short CMoralObserver::m_iMoralMatrix[][PLAYER_RACES::MAJOR6] = {0};
CString CMoralObserver::m_strTextMatrix[][PLAYER_RACES::MAJOR6] = {""};
...
void CMoralObserver::SerializeStatics(CArchive &ar)
{
	...
	if (ar.IsStoring())
	{
		for (int i = 0; i < EVENTNUMBER; i++)
			for (int j = 0; j < PLAYER_RACES::MAJOR6; j++)
			{
...
void CMoralObserver::CalculateEvents(std::vector<CSystem>& systems, const CString& sRaceID, BYTE byMappedRaceNumber)
{
	...
	AssertBotE(byMappedRaceNumber >= 1 && byMappedRaceNumber <= 6);
...
*//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*	short moralMatrix[][PLAYER_RACES::MAJOR6] = {
...
*/
	while(!m_iEvents.IsEmpty())
	{
		...
		for(std::vector<CSystem>::iterator it = systems.begin(); it != systems.end(); ++it)
			if(it->Majorized() && it->OwnerID() == sRaceID)
				it->SetMoral(moral);
...
{
/*	CString textMatrix[][PLAYER_RACES::MAJOR6] = {
...
/*
	...
	if (file.Open(fileName, CFile::modeCreate | CFile::modeWrite | CFile::typeText))
	{
		for (int j = 0; j < EVENTNUMBER; j++)
		{
			...
			s.Format("#%d\n", j);
			file.WriteString(s);
			for (int k = 0; k < PLAYER_RACES::MAJOR6; k++)
