...
void CSystem::Reset(bool call_up)
{
	...
	m_OwningStatus = OWNING_STATUS_EMPTY;
...
void CSystem::Serialize(CArchive &ar, bool end_of_round)
{
	...
	if (ar.IsStoring())
	{
		ar << static_cast<int>(m_OwningStatus);
		ar << m_dHabitants;
		ar << m_iMoral;
		ar << m_byBlockade;
		ar << m_iFoodBuildings;
	...
	if (ar.IsLoading())
	{
		int number = 0;
		ar >> number;
		m_OwningStatus = static_cast<OWNING_STATUS>(number);
		ar >> m_dHabitants;
		ar >> m_iMoral;
		ar >> m_byBlockade;
		ar >> m_iFoodBuildings;
...
*/
	...
	if (building->GetOnlyHomePlanet())
	{
		...
		{
			if (m_OwningStatus == OWNING_STATUS_TAKEN)
				return TRUE;
		}
		return FALSE;
	}
