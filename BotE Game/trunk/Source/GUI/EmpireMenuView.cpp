...
void CEmpireMenuView::OnDraw(CDC* dc)
{
	...
	if (pDoc->CheckTutorialShown(CBotEDoc::EMPIREMENU))
	{	
		Invalidate(FALSE);
...
void CEmpireMenuView::DrawEmpireTop5Menue(Gdiplus::Graphics *g)
{
	...
	for (list<CPoint>::const_iterator it = lSystems.begin(); it != lSystems.end(); ++it)
	{
		...
		if (pSector->GetFullKnown(pMajor->GetRaceID()) || pMajor->IsRaceContacted(pSector->OwnerID()) && pSector->GetScanned(pMajor->GetRaceID()))
		{
			// Planeten zeichnen
			...
			if (pMajor->IsRaceContacted(pSector->OwnerID()) || pMajor->GetRaceID() == pSector->OwnerID())
			{
				CMajor* pOwner = dynamic_cast<CMajor*>(pSector->Owner().get());
				if (pOwner)
				{
					...
					if (CRect(r.left+100,r.top+140+j*25,r.right-100,r.top+165+j*25).PtInRect(point))
					{
							...
							pMajor->GetEmpire()->GetMsgs()->GetAt(i).GetType() == EMPIRE_NEWS_TYPE::SOMETHING)
						{
							...
							if (p != CPoint(-1, -1))
							{
								...
								if (pDoc->GetSystem(p.x,p.y).OwnerID() == pMajor->GetRaceID())
								{
						...
						else if (pMajor->GetEmpire()->GetMsgs()->GetAt(i).GetType() == EMPIRE_NEWS_TYPE::MILITARY)
						{
							...
							if (p != CPoint(-1, -1))
							{
								...
								// Systemansicht anzeigen
								if (pMajor->GetEmpire()->GetMsgs()->GetAt(i).GetFlag() == 1 && pDoc->GetSystem(p.x,p.y).OwnerID() == pMajor->GetRaceID())
								{
									resources::pMainFrame->SelectMainView(VIEWS::SYSTEM_VIEW, pMajor->GetRaceID());
