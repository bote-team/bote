...
//////////////////////////////////////////////////////////////
	...
	if (pDoc->CheckTutorialShown(CBotEDoc::GALAXYMAP))
	{	
		Invalidate(FALSE);
		pDoc->GenerateTutorialBox("TGALAXYMAP");
	}
	if (pDoc->CheckTutorialShown(CBotEDoc::SHIPORDER))	
	{	
		Invalidate(FALSE);
...
void CGalaxyMenuView::GenerateGalaxyMap()
{
	...
	for (int y = 0; y < STARMAP_SECTORS_VCOUNT; y++)
		for (int x = 0; x < STARMAP_SECTORS_HCOUNT; x++)
		{
			...
			{
				g->DrawImage(m_mOwnerMark[pDoc->GetSystem(x,y).OwnerID()], pt.x, pt.y, STARMAP_SECTOR_WIDTH, STARMAP_SECTOR_HEIGHT);
			}
...
CString CGalaxyMenuView::CreateTooltip(void)
{
	...
	if (ko.is_in_rect(0, 0, STARMAP_SECTORS_HCOUNT, STARMAP_SECTORS_VCOUNT))
	{
		...
		if (pSector->GetSunSystem() && !pSector->Free() &&
			(pSector->GetScanPower(pMajor->GetRaceID()) > 50 || pSector->OwnerID() == pMajor->GetRaceID()))
		{
			...
			if (!mAllDefenceBuildings.empty())
			{
				...
				{
					CString sDefence;
					int onlines = mOnlineDefenceBuildings[it->first];
					sDefence.Format("%s: %d/%d\n", it->first, onlines, it->second);
